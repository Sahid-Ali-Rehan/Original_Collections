...
const EditProduct = () => {
    ...
    if (productId) {
      const fetchProduct = async () => {
        try {
          ...
          const response = await axios.get(`https://ruhana.onrender.com/api/products/details/${productId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`https://ruhana.onrender.com/api/products/update/${productId}`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });
