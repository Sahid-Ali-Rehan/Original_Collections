...
const EditProduct = () => {
    ...
    if (productId) {
      const fetchProduct = async () => {
        try {
          ...
          const response = await axios.get(`http://localhost:5000/api/products/details/${productId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://localhost:5000/api/products/update/${productId}`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });
